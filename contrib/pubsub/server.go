...
type Server struct {
	*booby.Server
	...
	clients map[*booby.Client]map[string]*TopicAgent
}
...
func (s *Server) invalid(ctx *booby.Context) bool {
	_, ok := ctx.Client.Get(userTopicKey)
	return !ok
}

func (s *Server) onAuthenticate(ctx *booby.Context) {
	defer util.Recover()
...
func (s *Server) onSubscribe(ctx *booby.Context) {
	defer util.Recover()
...
func (s *Server) onUnsubscribe(ctx *booby.Context) {
	defer util.Recover()
...
func (s *Server) onPublish(ctx *booby.Context) {
	defer util.Recover()
...
func (s *Server) onPublishToOne(ctx *booby.Context) {
	defer util.Recover()
...
// addClient .
func (s *Server) addClient(c *booby.Client) {
	c.Set(userTopicKey, &clientTopics{
		topicAgents: map[string]*TopicAgent{},
	})
}

func (s *Server) deleteClient(c *booby.Client) {
