...
type TopicAgent struct {
	...
	clients map[*booby.Client]util.Empty
}

// Add .
func (t *TopicAgent) Add(c *booby.Client) {
	t.mux.Lock()
...
// Delete .
func (t *TopicAgent) Delete(c *booby.Client) {
	t.mux.Lock()
	delete(t.clients, c)
	t.mux.Unlock()
}
...
// Publish .
func (t *TopicAgent) Publish(s *Server, from *booby.Client, topic *Topic) {
	msg := s.NewMessage(booby.CmdNotify, routePublish, topic.raw)
	t.mux.RLock()
	for to := range t.clients {
		err := to.PushMsg(msg, booby.TimeZero)
		if err != nil {
			if from != nil {
...
// PublishToOne .
func (t *TopicAgent) PublishToOne(s *Server, from *booby.Client, topic *Topic) {
	msg := s.NewMessage(booby.CmdNotify, routePublish, topic.raw)
	t.mux.RLock()
	for to := range t.clients {
		err := to.PushMsg(msg, booby.TimeZero)
