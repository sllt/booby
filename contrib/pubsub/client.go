...
func (c *Client) onPublish(ctx *booby.Context) {
	defer util.Recover()
...
func NewClient(dialer func() (net.Conn, error), args ...interface{}) (*Client, error) {
	var handler booby.Handler
	if len(args) > 0 {
		if h, ok := args[0].(booby.Handler); ok {
			handler = h.Clone()
		}
	}

	c, err := booby.NewClient(dialer, handler)
	if err != nil {
		return nil, err
	}
	...
	cli.Handler.HandleConnected(func(c *booby.Client) {
