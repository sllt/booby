...
function BoobyClient(url, codec, httpUrl, httpMethod) {
    var client = this;
    ...
    this._onMessage = function(event) {
        try {
            ...
            while (offset < event.data.byteLength) {
                ...
                if (methodLen == 0) {
                    console.log("[BoobyClient] onMessage: invalid request message with 0 method length, dropped");
                    return
                }
                ...
                switch (cmd) {
                    ...
                    case _CmdNotify:
                        ...
                        if (handler) {
                            ...
                            console.log("[BoobyClient] onMessage: invalid method: [%s], no handler", method);
                            return
                        }
                        break;
                    case _CmdResponse:
                        ...
                        if (session) {
                            ...
                            console.log("[BoobyClient] onMessage: session [%d] missing", seq);
                            return;
                        }
                        break;
                    default:
            ...
            console.log("[BoobyClient] onMessage: panic:", e);
        }
    }

    this.init = function() {
        console.log("[BoobyClient] init...");
        ...
        client.ws.onopen = function(event) {
            ...
            console.log("[BoobyClient] websocket onopen");
            if (client.onOpen) {
                client.onOpen(client);
            }
        };
        client.ws.onclose = function(event) {
            console.log("[BoobyClient] websocket onclose");
            if (client.onClose) {
        ...
        client.ws.onerror = function(event) {
            console.log("[BoobyClient] websocket onerror");
            if (client.onError) {
    ...
    try {
        ...
        console.log("[BoobyClient] init() failed:", e);
    }
}